...
for train, test in kf.split(dataset):
    ...
    # svm.setClassWeights(None)
    svm.setTermCriteria((cv2.TERM_CRITERIA_COUNT, 100, 1.e-06))
    svm.train(train_set, cv2.ml.ROW_SAMPLE, labels[train])
    predicted = svm.predict(test_set)[1]
    precision = accuracy_score(labels[test], predicted)
    precision_list.append(precision)
precision_cv_linear = np.mean(precision_list)
print("precision_cv_linear, 10-CV: ", precision_cv_linear)
...
print("rbf, 10-CV: ",precision_cv_rbf)
